...
test.describe('Learn flows', () => {
  test('learn hub routes explore filters and tools behind dedicated views', async ({ page }) => {
    ...
    await expect(page.getByText('Fingerpicking Foundations')).toBeVisible()
  ...
  test('canonical learn routes restore explore filters', async ({ soundgarden }) => {
    const page = await soundgarden.gotoHash('#/learn/explore?browse=skill&skill=fingerstyle')

    await expect(page.getByRole('link', { name: 'Explore', exact: true })).toHaveClass(
      /bg-emerald-600/
    )
